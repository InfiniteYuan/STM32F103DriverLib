...
Timer::Timer(TIM_TypeDef *timer, u8 Prioritygroup, u8 preemprionPriority, u8 subPriority, u16 second, u16 millisecond, u16 microsecond)
{
	...
	/*TIM_CounterMode_CenterAligned1/2/3 中央对齐模式123*/
...
u32 Timer::GetNowTime(void)
{
	return (mTempTimer->CNT*mPsc / 72);
}
...
u16 Timer::GetTimerArr()
{
	return mArr;
}

u16	Timer::GetTimerPsc()
{
	return mPsc;
}

void Timer::ResetTimer(u16 s, u16 ms, u16 us)
{
	TIM_Cmd(mTempTimer, DISABLE);//关闭定时器
	mTempTimer->CNT &= 0;		//定时器清零
	Conversion(s, ms, us);		//重设时间
	mTempTimer->ARR = mArr;
	mTempTimer->PSC = mPsc;
}
